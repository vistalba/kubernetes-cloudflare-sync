...
var options = struct {
    ...
    UseIpFromAnnotation: os.Getenv("USE_IP_FROM_ANNOTATION"),
}
func main() {
    ...
    flag.StringVar(&options.UseIpFromAnnotation, "use-ip-from-annotation", options.UseIpFromAnnotation, "use ip from annotation")
    flag.Parse()
    ...
    resync := func() {
        ...
        var ips []string
        if options.UseIpFromAnnotation != "" {
            for _, node := range nodes {
                if nodeIsReady(node) {
                    ips = append(ips, node.Annotations[options.UseIpFromAnnotation])
                }
            }
        }
        if len(ips) == 0 {
